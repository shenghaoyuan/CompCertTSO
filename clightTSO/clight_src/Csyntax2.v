...
(* This is the second half of Csyntax, from Csyntax2.v *)
...
(** Decidable equality on signedness, intsize, floatsize, and type *)
...
Scheme type_ind3 := Induction for type Sort Type
  with typelist_ind3 := Induction for typelist Sort Type
  with fieldlist_ind3 := Induction for fieldlist Sort Type.


Definition type_eq : forall (x: type) y, ({x=y} + {x<>y}).
Proof.
Ltac dec_eq_case_split t :=
  let X := fresh in
  let H := fresh in
  case t; intro X; 
  [rewrite X; clear X; try exact (left _ (refl_equal _)) 
   | right; intro H; apply X; inversion H; exact (refl_equal _)]. 
apply (type_ind3 (fun x, forall y, {x=y} + {x<>y})
                 (fun xl, forall y, {xl=y} + {xl<>y})
                 (fun xf, forall y, {xf=y} + {xf<>y}));
intros; case y; intros; try exact (left _ (refl_equal _)); try (right; discriminate);
repeat match goal with
| i : ?A, H: (forall(_ : ?A), _)|- _ => dec_eq_case_split (H i)
| i : intsize, j: intsize |- _       => dec_eq_case_split (intsize_eq i j)
| i : floatsize, j: floatsize |- _   => dec_eq_case_split (floatsize_eq i j)
| i : signedness, j: signedness |- _ => dec_eq_case_split (signedness_eq i j)
| i : ident, j: ident |- _           => dec_eq_case_split (ident_eq i j)
| i : Z, j: Z |- _                   => dec_eq_case_split (Z_eq_dec i j)
end.
Defined.
