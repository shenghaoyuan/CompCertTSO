...
with tr_condition (c: code): 
  ...
  | tr_CEcondition: forall map pr b ifso ifnot ns ntrue nfalse ntrue' nfalse' ntrue'' nfalse'',
      tr_condition c map pr b ns ntrue' nfalse' ->
...
Inductive tr_stmt (c: code) (map: mapping):
  ...
  | tr_Scall: forall optid sig b cl ns nd nexits ngoto nret rret rd n1 
                     rf n2 rargs,
  ...
  | tr_Sifthenelse: 
     forall a strue sfalse ns nd nexits ngoto nret rret ntrue nfalse ntrue' nfalse',
  ...
  | tr_Satomic: forall optid sop cl ns nd nexits ngoto nret rret rd n1 rargs,
     tr_exprlist c map nil cl ns n1 rargs ->
     c!n1 = Some (Iatomic sop rargs rd nd) ->
     reg_map_ok map rd optid ->
     tr_stmt c map (Satomic optid (*sig*) sop cl) ns nd nexits ngoto nret rret
  | tr_Sfence: forall ns nd nexits ngoto nret rret,
     c!ns = Some (Ifence nd) ->
  ...
  | tr_Sthread_create:
      ...
      tr_expr c map (r1 :: nil) earg  n1 n2 r2 None ->
      c ! n2 = Some (Ithreadcreate r1 r2 nd) ->
      tr_stmt c map (Sthread_create efn earg) ns nd nexits ngoto 
                  nret rret.
...
Inductive tr_function: CminorSel.function -> RTL.function -> Prop :=
  | tr_function_intro:
      ...
      tr_function f (RTL.mkfunction
                       f.(CminorSel.fn_sig)
                       rparams
                       (Int.repr (f.(CminorSel.fn_stackspace)))
                       code
                       nentry).
...
(* Conditions *)
  ...
  unfold add_nop in *.
  econstructor.
  eapply transl_condexpr_charact; try eapply add_instr_at; eauto with rtlg.
    apply tr_condition_incr with s1; auto.
    eapply transl_condexpr_charact; eauto with rtlg.
  apply tr_condition_incr with s0; auto.
  eapply transl_condexpr_charact; eauto with rtlg.
  by eapply instr_at_incr, add_instr_at; [|eassumption].
  by eapply instr_at_incr, add_instr_at; [|eassumption].
(* Lists *)
...
Lemma transl_stmt_charact:
  forall map stmt nd nexits ngoto nret rret s ns s' INCR
    (TR: transl_stmt false map stmt nd nexits ngoto nret rret s = OK ns s' INCR)
    (WF: map_valid map s)
    (OK: return_reg_ok s map rret),
  tr_stmt s'.(st_code) map stmt ns nd nexits ngoto nret rret.
Proof.
  induction stmt; intros; simpl in TR; pose proof TR as TR'; 
...
(*  (* Sbuiltin *)
  ...
  apply tr_expr_incr with s3; auto.
  eapply transl_expr_charact; eauto 4 with rtlg.
    eapply alloc_reg_target_ok; [| |edone];eauto 4 with rtlg.
    intros r IN. destruct IN as [<-|[]]; eauto 3 with rtlg. 
  intros r IN. destruct IN as [<-|[]]; eauto 3 with rtlg. 
  (* Satomic *)
  econstructor; eauto with rtlg.
  eapply tr_exprlist_incr with s'. apply state_incr_refl. 
  eapply transl_exprlist_charact; eauto with rtlg.
  eapply alloc_optreg_map_ok with (s1 := s0); eauto 3 with rtlg.
  (* Sfence *)
  econstructor. by apply add_instr_at in TR'. 
Qed.

Lemma transl_function_charact:
  forall f tf,
  transl_function false f = Errors.OK tf ->
  tr_function f tf.
Proof.
  intros until tf. unfold transl_function.
  caseEq (reserve_labels (fn_body f) (PTree.empty node, init_state)). 
  intros ngoto s0 RESERVE.
  caseEq (transl_fun false f ngoto s0). intros. 
  destruct (Zgt_bool (fn_stackspace f) Int.max_signed) as []_eqn : Z;
   intros.  inv H0. 
  destruct (Zlt_bool (fn_stackspace f) 0) as []_eqn : Z1;
   intros; inv H0.
  intros [nentry rparams] sfinal INCR TR E.
  destruct (Zgt_bool (fn_stackspace f) Int.max_signed). inv E.
  destruct (Zlt_bool (fn_stackspace f) 0) as []_eqn : Z1; inv E.
  monadInv TR.    
  exploit add_vars_valid. eexact EQ. apply init_mapping_valid.
  intros [A B]. 
  exploit add_vars_valid. eexact EQ1. auto. 
  intros [C D]. inversion EQ.
